...
class Custom_5_Fast_Quad_L2Net_Relu (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        PatchNet.__init__(self, **kw)
        self.downsample_factor = downsample_factor
        self._add_conv(  8*mchan,relu=True, )
        self._add_conv(  8*mchan,relu=True,  )
        self._add_conv( 16*mchan, k_pool = downsample_factor,relu=True,) # added avg pooling to decrease img resolution
        self._add_conv( 16*mchan,relu=False, gcu=False, selu=True)
        self._add_conv( 32*mchan,relu=True, stride=2, )
        # self._add_conv( 32*mchan,relu=True, )
        
        # replace last 8x8 convolution with 3 2x2 convolutions
        self._add_conv( 32*mchan, k=2, stride=2,relu=True, )
        # self._add_conv( 32*mchan, k=2, stride=2, relu=False, selu=relu22)
...
class Custom_5_Fast_Quad_L2Net_ConfCFS_Relu (Custom_5_Fast_Quad_L2Net_Relu):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_5_Fast_Quad_L2Net_Relu.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_7_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        self._add_conv(dim, k=2, stride=4, bn=False,relu=False, gcu=False, mish=True)
...
class Custom_7_Fast_Quad_L2Net_ConfCFS_Mish (Custom_7_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_7_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_8_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        self._add_conv(  8*mchan,relu=False, gcu=False, mish=True)
        self._add_conv(  8*mchan,relu=False, gcu=False, mish=True)
        self._add_conv( 16*mchan, k_pool = downsample_factor, mish=True, pool_type='avg') # added avg pooling to decrease img resolution
        self._add_conv( 16*mchan,relu=False, gcu=False, mish=True)
        self._add_conv( 32*mchan,relu=False, gcu=False, stride=2, mish=True)
        # self._add_conv( 32*mchan,relu=False, gcu=False, mish=True)
...
class Custom_8_Fast_Quad_L2Net_ConfCFS_Mish (Custom_8_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_8_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_9_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_9_Fast_Quad_L2Net_ConfCFS_Mish (Custom_9_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_9_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_10_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        # self._add_conv( 32*mchan, k=2, stride=2,relu=False, gcu=False, mish=True)
        # self._add_conv( 32*mchan, k=2, stride=2, relu=False, selu=relu22)
        self._add_conv(dim, k=8, stride=2, bn=False,relu=False, gcu=False, mish=True)
...
class Custom_10_Fast_Quad_L2Net_ConfCFS_Mish (Custom_10_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_10_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_11_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_11_Fast_Quad_L2Net_ConfCFS_Mish (Custom_11_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_11_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_12_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        # self._add_conv( 32*mchan, k=2, stride=2,relu=False, gcu=False, mish=True)
        # self._add_conv( 32*mchan, k=2, stride=2, relu=False, selu=relu22)
        self._add_conv(dim, k=4, stride=2, bn=False,relu=False, gcu=False, mish=True, dilation=2)
...
class Custom_12_Fast_Quad_L2Net_ConfCFS_Mish (Custom_12_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
...
class Custom_13_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_13_Fast_Quad_L2Net_ConfCFS_Mish (Custom_13_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_13_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_15_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_15_Fast_Quad_L2Net_ConfCFS_Mish (Custom_15_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_15_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_16_Fast_Quad_L2Net_Selu (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        self._add_conv(  8*mchan,relu=False, gcu=False, selu=True)
        self._add_conv(  8*mchan,relu=False, gcu=False, selu=True)
        self._add_conv( 16*mchan, k_pool = downsample_factor,relu=False, gcu=False, selu=True,pool_type='avg') # added avg pooling to decrease img resolution
        self._add_conv( 16*mchan,relu=False, gcu=False, selu=True)
        self._add_conv( 32*mchan,relu=False, gcu=False, stride=2, selu=True)
        # self._add_conv( 32*mchan,relu=False, gcu=False, selu=True)
...
class Custom_16_Fast_Quad_L2Net_ConfCFS_Selu (Custom_16_Fast_Quad_L2Net_Selu):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_16_Fast_Quad_L2Net_Selu.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_17_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
    def __init__(self, dim=128, mchan=4, relu22=False, downsample_factor=2, **kw ):
        ...
        # self._add_conv( 32*mchan, k=2, stride=2,relu=False, gcu=False, mish=True)
        # self._add_conv( 32*mchan, k=2, stride=2, relu=False, selu=relu22)
        self._add_conv(dim, k=5, dilation=4, stride=2, bn=False,relu=False, gcu=False, mish=True)
...
class Custom_17_Fast_Quad_L2Net_ConfCFS_Mish (Custom_17_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_15_Fast_Quad_L2Net_Mish.__init__(self, **kw)
...
class Custom_18_Fast_Quad_L2Net_Selu (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_18_Fast_Quad_L2Net_ConfCFS_Selu (Custom_16_Fast_Quad_L2Net_Selu):
    ...
    """
    def __init__(self, **kw ):
        Custom_18_Fast_Quad_L2Net_Selu.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_19_Fast_Quad_L2Net_Selu (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_19_Fast_Quad_L2Net_ConfCFS_Selu (Custom_16_Fast_Quad_L2Net_Selu):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_19_Fast_Quad_L2Net_Selu.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_20_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_20_Fast_Quad_L2Net_ConfCFS_Mish (Custom_20_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_20_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_21_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_21_Fast_Quad_L2Net_ConfCFS_Mish (Custom_21_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_21_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_22_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_22_Fast_Quad_L2Net_ConfCFS_Mish (Custom_22_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_22_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
...
class Custom_23_Fast_Quad_L2Net_Mish (PatchNet):
    """ Faster version of Quad l2 net, replacing one dilated conv with one pooling to diminish image resolution thus increase inference time
    Dilation  factors and pooling:
        1,1,1, pool2, 1,1, 2,2, 4, 8, upsample2
    """
...
class Custom_23_Fast_Quad_L2Net_ConfCFS_Mish (Custom_22_Fast_Quad_L2Net_Mish):
    """ Fast r2d2 architecture
    """
    def __init__(self, **kw ):
        Custom_23_Fast_Quad_L2Net_Mish.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
    ...
    def forward_one(self, x):
        ...
        ureliability = self.clf(x**2)
        # urepeatability = self.sal(x**2)
        urepeatability = self.sal(descriptors[2]**2)

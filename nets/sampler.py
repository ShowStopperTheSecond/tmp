...
class NghSampler2PNP(nn.Module):
    ...
    def forward(self, feats, confs, aflow, **kw):
        ...
        anchor = feat1
        positive_samples = feat2[b2, :, xy2p[1], xy2p[0]]
        negative_samples = feat2[b2, :, xy2n[1], xy2n[0]]
        distractor_samples =  feat2[b3, :, y3, x3]
        return scores, gt, mask, qconf, anchor, positive_samples, negative_samples, distractor_samples

class SubSamplerPNP(FullSampler):
    """ pixels are selected in an uniformly spaced grid
    """
    ...
    def __call__(self, feats, confs, aflow):
        ...
        # compute all images together
        queries = feat1.reshape(B, D, -1)  # B x D x (Hq x Wq)
        queries = queries / torch.norm(queries, keepdim=True)
        database = feat2.reshape(B, D, -1)  # B x D x (Hd x Wd)
        database = database / torch.norm(database, keepdim=True)
...
class DoubleDescNghSampler2(nn.Module):
    """ Similar to NghSampler, but doesnt warp the 2nd image.
    Distance to GT =>  0 ... pos_d ... neg_d ... ngh
    Pixel label    =>  + + + + + + 0 0 - - - - - - -
    ...
    def __init__(self, ngh, subq=1, subd=1, pos_d=0, neg_d=2, border=None,
                 maxpool_pos=True, subd_neg=0, desc_dim=128):
        nn.Module.__init__(self)
        assert 0 <= pos_d < neg_d <= (ngh if ngh else 99)
        self.ngh = ngh
        self.pos_d = pos_d
    ...
    def forward(self, feats, confs, aflow, **kw):
        ...
        # compute positive scores
        xy2p = clamp(xy2[:, None, :] + self.pos_offsets[:, :, None])
        pscores1 = (feat1[None, :, :self.desc_dim] * feat2[b2, :self.desc_dim, xy2p[1], xy2p[0]]).sum(dim=-1).t()
        pscores2 = (feat1[None, :, self.desc_dim:] * feat2[b2, self.desc_dim:, xy2p[1], xy2p[0]]).sum(dim=-1).t()
        pscores = torch.cat([pscores1, pscores2])
        #        xy1p = clamp(torch.stack((x1,y1))[:,None,:] + self.pos_offsets[:,:,None])
        #        grid = FullSampler._aflow_to_grid(aflow)
        #        feat2p = F.grid_sample(feat2, grid, mode='bilinear', padding_mode='border')
        #        pscores = (feat1[None,:,:] * feat2p[b1,:,xy1p[1], xy1p[0]]).sum(dim=-1).t()
        if self.maxpool_pos:
            ...
            if confs:
                sel1 = clamp(xy2 + self.pos_offsets[:, pos1.view(-1)])
                qconf1 = (qconf + conf2[b2, :, sel1[1], sel1[0]].view(shape)) / 2
                sel2 = clamp(xy2 + self.pos_offsets[:, pos2.view(-1)])
                qconf2 = (qconf + conf2[b2, :, sel2[1], sel2[0]].view(shape)) / 2
                qconf = torch.concat([qconf1, qconf2])

        # compute negative scores
        xy2n = clamp(xy2[:, None, :] + self.neg_offsets[:, :, None])
        nscores1 = (feat1[None, :, :self.desc_dim] * feat2[b2, :self.desc_dim, xy2n[1], xy2n[0]]).sum(dim=-1).t()
        nscores4 = (feat1[None, :, :self.desc_dim] * feat2[b2, self.desc_dim:, xy2p[1], xy2p[0]]).sum(dim=-1).t()

        nscores2 = (feat1[None, :, self.desc_dim:] * feat2[b2, self.desc_dim:, xy2n[1], xy2n[0]]).sum(dim=-1).t()
        nscores3 = (feat1[None, :, self.desc_dim:] * feat2[b2, :self.desc_dim, xy2p[1], xy2p[0]]).sum(dim=-1).t()

        nscores = torch.cat([torch.cat([nscores1, nscores4], 1), torch.cat([nscores2, nscores3], 1)])
        if self.sub_d_neg:
            # add distractors from a grid
            b3, y3, x3, _ = self.gen_grid(self.sub_d_neg, aflow)
            distractors11 = feat2[b3, :self.desc_dim, y3, x3]
            distractors12 = feat2[b3, self.desc_dim:, y3, x3]
            feat11 = feat1[:, :self.desc_dim]
            feat12 = feat1[:, self.desc_dim:]
            dscores1 = torch.matmul(feat11, distractors11.t())
            dscores2 = torch.matmul(feat11, distractors12.t())
            dscores3 = torch.matmul(feat12, distractors11.t())
            dscores4 = torch.matmul(feat12, distractors12.t())

            del distractors11, distractors12

            # remove scores that corresponds to positives or nulls
            dis2 = (x3 - xy2[0][:, None]) ** 2 + (y3 - xy2[1][:, None]) ** 2
            dis2 += (b3 != b2[:, None]).long() * self.neg_d ** 2
            dscores1[dis2 < self.neg_d ** 2] = 0
            dscores2[dis2 < self.neg_d ** 2] = 0
            dscores3[dis2 < self.neg_d ** 2] = 0
            dscores4[dis2 < self.neg_d ** 2] = 0

            dscores = torch.cat([torch.cat([dscores1, dscores2], 1), torch.cat([dscores3, dscores4], 1)])
